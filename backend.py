...
def get_database_url():
    ...
    if not database_url:
        raise ValueError(
            ...
            "Please add your Neon PostgreSQL Database URL (from https://console.neon.tech) to .env"
        )

    # Normalize url scheme if necessary (psycopg prefers postgresql://)
...
DEFAULT_ORIGIN = os.getenv("DEFAULT_ORIGIN", "Kolkata")
...
def _empty_constraints() -> dict[str, Any]:
    return {
        "destination": "",
        "origin": f"{DEFAULT_ORIGIN} ({DEFAULT_ORIGIN_IATA})",
        "duration": "",
        "budget": "",
        "travel_style": "",
        "special_preferences": [],
...
Generate:
1. Likely departure airport (default to {origin} if not explicitly specified by user)
...
def flight_agent(state: TravelState):
    ...
    query = state["user_query"]
    origin = (
        state.get("trip_constraints", {}).get("origin")
        or f"{DEFAULT_ORIGIN} ({DEFAULT_ORIGIN_IATA})"
    )
...
Budget Results:
{str(state.get('budget_results', ''))[:600]}
